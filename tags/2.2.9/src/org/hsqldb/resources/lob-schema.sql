...
CREATE SCHEMA SYSTEM_LOBS AUTHORIZATION DBA
 ...
 CREATE PROCEDURE DELETE_UNUSED(L_IDS BIGINT ARRAY)
  ...
  REPEAT
   INSERT INTO TEMP_IDS SELECT LOB_ID FROM LOB_IDS WHERE LOB_USAGE_COUNT < 1 AND LOB_ID NOT IN (SELECT * FROM LIVE_IDS) LIMIT 1000;
   ...
   DELETE FROM LOBS WHERE LOB_ID
    IN (SELECT TEMP_ID FROM TEMP_IDS);
   DELETE FROM LOB_IDS WHERE LOB_ID IN (SELECT TEMP_ID FROM TEMP_IDS);
 ...
 CREATE PROCEDURE DELETE_UNUSED_LOBS(LIMIT_ID BIGINT)
  ...
  REPEAT
   INSERT INTO TEMP_IDS SELECT LOB_ID FROM LOB_IDS WHERE LOB_USAGE_COUNT = 0 AND LOB_ID < LIMIT_ID LIMIT 1000;
   INSERT INTO BLOCKS (SELECT BLOCK_ADDR,BLOCK_COUNT,0 FROM LOBS WHERE LOB_ID
    IN (SELECT TEMP_ID FROM TEMP_IDS));
   DELETE FROM LOBS WHERE LOB_ID
    IN (SELECT TEMP_ID FROM TEMP_IDS);
